...
export default function AdminTelemedicine() {
  ...
  const endSession = async (sessionId) => {
    try {
      ...
      const detail = err.response?.data?.detail;
      setToast({ message: typeof detail === 'string' ? detail : 'Failed to end session', type: 'error' });
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto py-4 px-4 sm:px-0">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">View and manage all virtual consultations</p>
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <p className="text-sm text-gray-500">There are currently no telemedicine sessions.</p>
        </div>
      ) : (
          ...
          {sessions.map(session => (
            ...
            >
              ...
              <div className="text-sm text-gray-700 mb-4 space-y-1">
                ...
                {session.created_by && (
                  <div className="flex">
                    <span className="w-28 text-gray-500 font-medium">Created By:</span>
                    <span className="text-gray-900">Dr. (ID: {session.created_by})</span>
                  </div>
                )}
